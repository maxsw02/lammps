// clang-format off
/* ----------------------------------------------------------------------
 LAMMPS - Large-scale Atomic/Molecular Massively Parallel Simulator
 https://www.lammps.org/, Sandia National Laboratories
 LAMMPS development team: developers@lammps.org

 Copyright (2003) Sandia Corporation.  Under the terms of Contract
 DE-AC04-94AL85000 with Sandia Corporation, the U.S. Government retains
 certain rights in this software.  This software is distributed under
 the GNU General Public License.

 See the README file in the top-level LAMMPS directory.
 ------------------------------------------------------------------------- */

/* ----------------------------------------------------------------------
   Contributing author:
    Max Win (UPENN)
    references: Espanol and Revenga, Phys Rev E 67, 026705 (2003)
------------------------------------------------------------------------- */

#include "pair_sdpd_ideal.h"

#include "atom.h"
#include "comm.h"
#include "domain.h"
#include "error.h"
#include "force.h"
#include "memory.h"
#include "neigh_list.h"
#include "neighbor.h"
#include "update.h"

#include <cmath>

#ifndef USE_ZEST
#include "random_mars.h"

#include <vector>
#include <numeric>
#endif


using namespace LAMMPS_NS;
using namespace std;
static const double sqrt_2_inv = std::sqrt(0.5);

/* ---------------------------------------------------------------------- */

PairSDPDIdeal::PairSDPDIdeal (LAMMPS *lmp): Pair (lmp), random(nullptr) 
{
  restartinfo = 0;
  manybody_flag = 1;
  d = nullptr;
  //d = F = nullptr;
  nmax = 0;

  comm_forward = 1;
  comm_reverse = 1;
}

/* ---------------------------------------------------------------------- */

PairSDPDIdeal::~PairSDPDIdeal () {
  
  if (copymode) return;
  memory->destroy(d);
  //memory->destroy(F);

  if (allocated) {
    memory->destroy (setflag);
    memory->destroy (cutsq);

    memory->destroy (cut);

  }
  delete random;
}

/* ---------------------------------------------------------------------- */

void PairSDPDIdeal::compute (int eflag, int vflag) {
  int i, j, ii, jj, inum, jnum, itype, jtype;
  double xtmp, ytmp, ztmp, delx, dely, delz, ex,ey,ez, h, Pi, Pj;
  double therm, eta, visc,kb;
  double fpgradx, fpgrady,fpgradz, fviscx, fviscy, fviscz, fvisc2x, fvisc2y, fvisc2z;
  double dij, comb_temp, fdd;
  double aij, Aij, bij, Bij, cij, Cij;
  double term1, term2, term3, term4; //Entropy terms
  double h, ih, ihsq, ihc, velx, vely, velz;
  double rsq, tmp, wfd, VijdotEij, W, vdotv, trace;
  double prefactor, wiener[3][3],sym_wiener[3][3], f_random[3], f_rand[3];

  if (atom->nmax > nmax) {
    memory->destroy(d);
    //memory->destroy(F);
    nmax = atom->nmax;
    memory->create(d,nmax,"pair:d");
    //memory->create(F,nmax,"pair:F");
  }
  
  int *ilist, *jlist, *numneigh, **firstneigh;
  double vxtmp, vytmp, vztmp, imass, jmass, fi, fj, fvisc, dV, term_rand, m;
  
  
  //ev_init(eflag, vflag);

  double **v = atom->vest;
  double **x = atom->x;
  double **f = atom->f;
  double *mass = atom->mass;
  double *cv = atom->cv;
  int *type = atom->type;
  int nlocal = atom->nlocal;
  int newton_pair = force->newton_pair;
  int dimension = domain->dimension;
  double dtinv = 1.0 / update->dt;
  double kBoltzmann = force->boltz;
  int nlocal = atom->nlocal;
  int nall = nlocal + atom->nghost;

  double *entropy = atom->entropy;
  double *dentropy = atom->dentropy;
  double *temperature = atom->temperature;

  inum = list->inum;
  ilist = list->ilist;
  numneigh = list->numneigh;
  firstneigh = list->firstneigh;

  visc = therm = eta = 1; //placeholders


  //Calculate density of each atom
  if (newton_pair) {
    for (i = 0; i < nall; i++) 
      d[i] = 0.0;
      //F[i] = 0.0; // gradient of W and W because d =W
      entropy[i] = -1.0; //setting initial temperature
      cv[i] = 1.5;
  } else for (i = 0; i < nlocal; i++) 
      d[i] = 0.0;
      //F[i] = 0.0; // gradient of W and W because d =W

      entropy[i] = -1.0; //setting initial temperature
      cv[i] = 1.5; 
  // rho = density at each atom
  // loop over neighbors of my atoms

  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
    xtmp = x[i][0];
    ytmp = x[i][1];
    ztmp = x[i][2];
    itype = type[i];
    jlist = firstneigh[i];
    jnum = numneigh[i];

    for (jj = 0; jj < jnum; jj++) {
      j = jlist[jj];
      j &= NEIGHMASK;

      delx = xtmp - x[j][0];
      dely = ytmp - x[j][1];
      delz = ztmp - x[j][2];
      rsq = delx*delx + dely*dely + delz*delz;

      if (rsq < cutsq[itype][jtype]) { //unsure
        jtype = type[j];
        h = cut[itype][jtype]; 
        ih = 1.0 / h;
        ihsq = ih * ih;
        ihc = ih * ih * ih;

        wfd = h - sqrt(rsq);
        W = h + 3*sqrt(rsq);
        if (domain->dimension == 3) {
          // Lucy Kernel, 3d
          // Note that wfd, the derivative of the weight function with respect to r,
          // is lacking a factor of r.
          // The missing factor of r is recovered by
          // (1) using delV . delX instead of delV . (delX/r) and
          // (2) using f[i][0] += delx * fpair instead of f[i][0] += (delx/r) * fpair
          //wfd = -25.066903536973515383e0 * wfd * wfd * ihsq * ihsq * ihsq * ih;
          // Regular lucy function not the derivative
          W = 2.08890862808 * ihc * W * wfd * wfd * wfd * ihc * ihc * ih;  
        } else {
          // Lucy Kernel, 2d
          //wfd = -19.098593171027440292e0 * wfd * wfd * ihsq * ihsq * ihsq;
          W = 2.08890862808 * ihc * W * wfd * wfd * ihc * ihc ; //THIS IS A PLACEHOLDER WRONG
          }
        d[i] += W;
        //F[i] += wfd; 
        }
      }
    }
  

  // loop over neighbors of my atoms
  if (newton_pair) comm->reverse_comm(this);
  //Might need to put some stuff here for entropy and pressure variables
  //comm->forward_comm(this);


  for (ii = 0; ii < inum; ii++) {
    i = ilist[ii];
    xtmp = x[i][0];
    ytmp = x[i][1];
    ztmp = x[i][2];
    vxtmp = v[i][0];
    vytmp = v[i][1];
    vztmp = v[i][2];
    itype = type[i];
    jlist = firstneigh[i];
    jnum = numneigh[i];

    imass = mass[itype];
    
    temperature[i] = 1/(6.28318530718) * pow(1/1, 2/3) * d[i] * exp((2*entropy[i]-5) / 3); // 1/1 should be 1/kb
    Pi = d[i] * temperature[i];

    fpgradx = fpgrady = fpgradz = 0;
    fviscx = fviscy = fviscz = fvisc2x = fvisc2y = fvisc2z = 0;

    for (jj = 0; jj < jnum; jj++) {
      j = jlist[jj];
      j &= NEIGHMASK;

      delx = xtmp - x[j][0];
      dely = ytmp - x[j][1];
      delz = ztmp - x[j][2];
      ex = delx / abs(delx);
      ey = dely / abs(dely);
      ez = delz / abs(delz);

      rsq = delx * delx + dely * dely + delz * delz;
      jtype = type[j];
      jmass = mass[jtype];

      if (rsq < cutforcesq) {
        h = cut[itype][jtype];
        ih = 1.0 / h;
        ihsq = ih * ih;

        double r = sqrt (rsq);
        wfd = h - r;
        //W = h + 3*r; 
        if (domain -> dimension == 3) {
          // Lucy Kernel, 3d
          // This F function in the espanol paper
          // gradW = dW/dr = -r * F(r)
          wfd = 25.066903536973515383e0 * wfd * wfd * ihsq * ihsq * ihsq * ih;
          // Regular lucy function not the derivative
        } else {
          // Lucy Kernel, 2d
          wfd = -19.098593171027440292e0 * wfd * wfd * ihsq * ihsq * ihsq;
        }

        // compute pressure  of atom j with Tait EOS
        temperature[j] = 1/(6.28318530718) * pow(1/1, 2/3) * d[j] * exp((2*entropy[j]-5) / 3); // 1/1 should be 1/kb
        Pj = d[j] * temperature[j];
        comb_temp = (temperature[i]* temperature[j]) /(temperature[i] + temperature[j]);
        dij = (1/cv[i] * 1/cv[j]) * comb_temp /(temperature[i] + temperature[j]);

        velx=vxtmp - v[j][0];
        vely=vytmp - v[j][1];
        velz=vztmp - v[j][2];

        // dot product of velocity delta and ex,y,z ector
        VijdotEij = ex * velx + ey * vely + ez * velz;
        

        fdd = wfd / (d[i]* d[j]);
        
        // Stochastic constants
        
        Aij = sqrt(8 * comb_temp * kb * (((5/3) * visc - eta))*fdd);
        aij = Aij * Aij / (8 * kb) *(1/temperature[i] + 1/temperature[j]);
        
        Bij = sqrt(8 * comb_temp * kb * (((5/3) * visc + 8 * eta))*fdd);
        bij = Bij * Bij / (12 * kb) *(1/temperature[i] + 1/temperature[j]);

        Cij = sqrt(4 * therm * kb * temperature[i]* temperature[j] * fdd);
        cij = Cij * Cij / (2 * kb * temperature[i]* temperature[j]);

        //forces 1,2,3 terms in Eq 63 of Espanol
        fviscx = (1 - dij) * aij * velx;
        fviscy = (1 - dij) * aij * vely;
        fviscz = (1 - dij) * aij * velz;

        fvisc2x = (1 - dij) * (aij/3 + bij) * ex * VijdotEij; 
        fvisc2y = (1 - dij) * (aij/3 + bij) * ey * VijdotEij;
        fvisc2y = (1 - dij) * (aij/3 + bij) * ez * VijdotEij;

        fpgradx = delx * wfd * (Pi / d[i] + Pj / d[j]);
        fpgrady = dely * wfd * (Pi / d[i] + Pj / d[j]);
        fpgradz = delz * wfd * (Pi / d[i] + Pj / d[j]);


        //entropy
        vdotv = velx * velx + vely * vely + velz * velz;
        term1 = (aij * vdotv + (aij/3 + bij) * VijdotEij * VijdotEij) * (1 - dij - temperature[j] /(temperature[i] + temperature[j]) * kb/cv[i]);

        term3 = fdd * (temperature[i] - temperature[j]);

        if (i != j) {
          term2 = comb_temp * (10/3 * aij * bij);
          term4 = fdd * temperature[j];
        }
        // random force and entropy calculation
        // independent increments of a Wiener process matrix
#ifdef USE_ZEST
        wiener[0][0] = gaussian (generator);
        wiener[1][1] = gaussian (generator);
        wiener[2][2] = gaussian (generator);

        wiener[0][1] = wiener[1][0] = sqrt_2_inv * gaussian (generator);
        wiener[0][2] = wiener[2][0] = sqrt_2_inv * gaussian (generator);
        wiener[1][2] = wiener[2][1] = sqrt_2_inv * gaussian (generator);
#else
        wiener[0][0] = random->gaussian ();
        wiener[1][1] = random->gaussian ();
        wiener[2][2] = random->gaussian ();
        wiener[0][0] = wiener[0][1] = wiener[1][0] = random->gaussian ();
        wiener[1][1] = wiener[0][2] = wiener[2][0] = random->gaussian ();
        wiener[2][2] = wiener[1][2] = wiener[2][1] = random->gaussian ();
        dV = random->gaussian ();

        //wiener[0][1] = wiener[1][0] = sqrt_2_inv * random->gaussian ();
        //sqrt_2_inv * random->gaussian ();
        //wiener[1][2] = wiener[2][1] = sqrt_2_inv * random->gaussian ();
#endif
        trace = wiener[0][0] + wiener[1][1] + wiener[2][2];
        sym_wiener[0][0] = 0.5 * (wiener[0][0] + wiener[0][0]) - 1/3 * trace;  
        sym_wiener[1][1] = 0.5 * (wiener[1][1] + wiener[1][1]) - 1/3 * trace;
        sym_wiener[2][2] = 0.5 * (wiener[2][2] + wiener[2][2]) - 1/3 * trace;
        
        sym_wiener[1][0] = sym_wiener[0][1] = 0.5 * (wiener[0][1] + wiener[1][0]);
        sym_wiener[2][1] = sym_wiener[1][2] = 0.5 * (wiener[1][2] + wiener[2][1]);
        sym_wiener[2][0] = sym_wiener[0][2] = 0.5 * (wiener[0][2] + wiener[2][0]);

        //prefactor = sqrt (-4. * kBoltzmann*temperature * fvisc * dtinv) / r;

        f_rand[0] = (Aij * sym_wiener[0][0] + Bij * (1/3) * trace) * ex + (Aij * sym_wiener[0][1] + Bij * (1/3) * trace) * ey + (Aij * sym_wiener[0][2] + Bij * (1/3) * trace) * ez;
        f_rand[1] = (Aij * sym_wiener[1][0] + Bij * (1/3) * trace) * ex + (Aij * sym_wiener[1][1] + Bij * (1/3) * trace) * ey + (Aij * sym_wiener[1][2] + Bij * (1/3) * trace) * ez;
        f_rand[2] = (Aij * sym_wiener[2][0] + Bij * (1/3) * trace) * ex + (Aij * sym_wiener[2][1] + Bij * (1/3) * trace) * ey + (Aij * sym_wiener[2][2] + Bij * (1/3) * trace) * ez;
        
        f_random[0] = f_rand[0];
        f_random[1] = f_rand[1];
        f_random[2] = f_rand[2];

        term_rand = -0.5 * (f_rand[0] * velx + f_rand[1] * vely + f_rand[2] * velz) + Cij * dV;

        
        f[i][0] += fpgradx - fviscx - fvisc2x + f_rand[0];
        f[i][1] += fpgrady - fviscy - fvisc2y + f_rand[1];
        f[i][2] += fpgradz - fviscz - fvisc2z + f_rand[2];

        dentropy[i] += 0.5 * term1 - 2 * kb / m * term2 - 2 * therm * term3 - 2 * therm * term4 + term_rand;

        if (newton_pair || j < nlocal) {
          f[i][0] -= fpgradx - fviscx - fvisc2x + f_rand[0];
          f[i][1] -= fpgrady - fviscy - fvisc2y + f_rand[1];
          f[i][2] -= fpgradz - fviscz - fvisc2z + f_rand[2];

          dentropy[i] -= 0.5 * term1 - 2 * kb / m * term2 - 2 * therm * term3 - 2 * therm * term4 + term_rand;
        }

        //if (evflag)
        //  ev_tally (i, j, nlocal, newton_pair, 0.0, 0.0, f, delx, dely, delz);
      }
    }
  }

  //if (vflag_fdotr) virial_fdotr_compute ();
}

/* ----------------------------------------------------------------------
 allocate all arrays
 ------------------------------------------------------------------------- */

void PairSDPDIdeal::allocate () {
  allocated = 1;
  int n = atom->ntypes;

  memory->create (setflag, n + 1, n + 1, "pair:setflag");
  for (int i = 1; i <= n; i++)
    for (int j = i; j <= n; j++)
      setflag[i][j] = 0;

  memory->create (cutsq, n + 1, n + 1, "pair:cutsq");
  memory->create (cut, n + 1, n + 1, "pair:cut");
}

/* ----------------------------------------------------------------------
 global settings
 ------------------------------------------------------------------------- */

void PairSDPDIdeal::settings (int narg, char **arg) {
  if (narg != 1)
    error->all (FLERR, "Illegal number of arguments for "
                "pair_style sdpd/ideal");

  
  //temperature = utils::numeric(FLERR, arg[0], false, lmp);
  //viscosity = utils::numeric(FLERR, arg[1], false, lmp);
  
  // seed is immune to underflow/overflow because it is unsigned
  seed = comm->nprocs + comm->me + atom->nlocal;
  if (narg == 1) seed += utils::inumeric(FLERR, arg[2], false, lmp);
#ifdef USE_ZEST
  generator.seed (seed);
#else
  random = new RanMars (lmp, seed);
#endif
}

/* ----------------------------------------------------------------------
 set coeffs for one or more type pairs
 ------------------------------------------------------------------------- */

void PairSDPDIdeal::coeff (int narg, char **arg) {
  //double *entropy = atom->entropy;
  if (narg != 3)
    error->all (FLERR, "Incorrect args for pair_style "
                "sdpd/ideal coefficients");

  if (!allocated) allocate();

  int ilo, ihi, jlo, jhi;
  utils::bounds(FLERR, arg[0], 1, atom->ntypes, ilo, ihi, error);
  utils::bounds(FLERR, arg[1], 1, atom->ntypes, jlo, jhi, error);

  double cut_one = utils::numeric(FLERR,arg[2], false, lmp);

  if (cut_one <= 0) error->all (FLERR, "Cutoff must be positive");

  int count = 0;
  for (int i = ilo; i <= ihi; i++) {
    for (int j = MAX(jlo,i); j <= jhi; j++) {
      cut[i][j] = cut_one;
      setflag[i][j] = 1;
      count++;
    }
  }

  if (count == 0)
    error->all(FLERR,"Incorrect args for pair coefficients");
}

/* ----------------------------------------------------------------------
 init specific to this pair style
------------------------------------------------------------------------- */

void PairSDPDIdeal::init_style()
{
  if ((!atom->dentropy_flag) || (atom->dentropy == nullptr))
    error->all(FLERR,"Pair style sdpd/ideal requires atom attributes entropy and dentropy");

  neighbor->add_request(this);
}

/* ----------------------------------------------------------------------
 init for one type pair i,j and corresponding j,i
 ------------------------------------------------------------------------- */

double PairSDPDIdeal::init_one (int i, int j) {
  if (setflag[i][j] == 0)
    error->all(FLERR,"Not all pair sdpd/ideal coeffs are set");

  cut[j][i] = cut[i][j];

  return cut[i][j];
}


void *PairSDPDIdeal::extract_peratom(const char *str, int &ncol)
{
  if (strcmp(str,"d") == 0) {
    ncol = 0;
    return (void *) d;
  //} else if (strcmp(str,"F") == 0) {
  //  ncol = 0;
  //  return (void *) F;
  }

  return nullptr;
}

int PairSDPDIdeal::pack_reverse_comm(int n, int first, double *buf)
{
  int i,m,last;

  m = 0;
  last = first + n;
  for (i = first; i < last; i++) buf[m++] = d[i];
  return m;
}

void PairSDPDIdeal::unpack_reverse_comm(int n, int *list, double *buf)
{
  int i,j,m;

  m = 0;
  for (i = 0; i < n; i++) {
    j = list[i];
    d[j] += buf[m++];
  }
}

double PairSDPDIdeal::memory_usage()
{
  double bytes = (double)maxeatom * sizeof(double);
  bytes += (double)maxvatom*6 * sizeof(double);
  bytes += (double)2 * nmax * sizeof(double);
  return bytes;
}